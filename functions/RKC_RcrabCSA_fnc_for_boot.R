#K.Palof ADF&G 
# updated 10-3-16
# Just function for bootstrap for red crab.  This function is modified slightly to speed up the bootstrap process

# load this function before starting bootstraps.
...
RcrabCSA1B <- function (year = NULL, catch = NULL, preR = NULL, recr = NULL, 
...
{  
  ...
  lower <- c(rep(0.001, yrs), 0.001,0.001,0.01, 0.1) #creates a vector with 21 spaces, all 1 except the last is 0 
  ...
  cs <- function(x) {
    ...
    TPRSS <- sum((((log(preR[seq(1, yrs, 1)]+0.001) - 
                    log(prest[seq(1, yrs, 1)]+0.001)) ^2)*w[seq(1,yrs,1)]), na.rm = T)
  ...
  outs <- optim(parms, cs, gr = NULL, lower = lower, upper = upper, 
                ...
                hessian = F) #changed call to Hessian as F to save time in bootstrap
  ...
  out3 <- cbind(q, s)
  out6 <- cbind(outs$par) #, lower_CI, upper_CI - removed these to speed up bootstrap
  
  
  #output <- list(out2, out3, out4, out5, out1, out6, outs)
  output <- list(estimates = out2, parms = out3, SSQ=out1, Parest = outs)
